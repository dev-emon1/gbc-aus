...
export default function SavingTips() {
  const section = COST_OF_LIVING_CONTENT.tips;

  return (
    <section className="relative overflow-hidden bg-slate-50 py-24 lg:py-32">
      <div className="absolute -left-20 top-10 h-80 w-80 rounded-full bg-[#D84D95]/5 blur-[140px]" />
          ...
          {section.items.map((item, index) => {
            ...
            return (
              <MotionReveal key={item.title} delay={index * 0.05}>
                <article className="group h-full rounded-[30px] border border-slate-200 bg-white p-8 shadow-sm transition-all duration-500 hover:-translate-y-2 hover:border-[#D84D95]/20 hover:shadow-[0_35px_90px_rgba(15,23,42,.08)]">
