...
export default function SupportServices() {
  const section = SUPPORT_WELFARE_CONTENT.services;
      ...
      <div className={CONTAINER}>
        ...
        <MotionStagger className="mt-16 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {section.items.map((item, index) => {
            const Icon =
              ICONS[item.icon as keyof typeof ICONS] ?? HeartHandshake;

            return (
              <MotionReveal key={item.title} delay={index * 0.08}>
                <div className="group flex h-full flex-col rounded-[30px] border border-slate-200 bg-white p-7 transition-all duration-300 hover:-translate-y-2 hover:border-[#D84D95]/20 hover:shadow-[0_25px_60px_rgba(15,23,42,.08)]">
                  ...
                  <div className="mt-8 flex items-center justify-between border-t border-slate-100 pt-5">
                    ...
                    <span className="rounded-full bg-[#B7D531]/15 px-3 py-1 text-xs font-semibold text-[#6E8E1D]">
                      Available
                    </span>
                  </div>
                </div>
